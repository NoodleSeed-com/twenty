...
describe('SdkProviderFactoryService', () => {
  ...
  describe('Google Vertex provider', () => {
    ...
    // Vertex serves the same Gemini models as the Generative AI API, so
    // dropping this middleware would break tool results in a way no other
    // assertion here would notice: the mock returns the model unwrapped.
    it('should wrap models with the Gemini tool-result sanitisation', () => {
      const instance = service.createProvider('vertex', VERTEX_CONFIG);

      instance.createModel('gemini-2.5-pro');
