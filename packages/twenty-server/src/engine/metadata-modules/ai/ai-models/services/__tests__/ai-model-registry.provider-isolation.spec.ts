...
describe('AiModelRegistryService provider isolation', () => {
  it('keeps valid providers available when a malformed provider fails to construct', async () => {
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        { provide: AiModelPreferencesService, useValue: preferencesService },
        { provide: ConfigGroupHashService, useValue: configGroupHashService },
      ],
    })
      // Resolve by token so the test also works on older stable releases
      // that predate the custom-provider entitlement dependency.
      .useMocker((token) => {
        if (
          typeof token === 'function' &&
          token.name === 'CustomAiProviderAccessService'
        ) {
          return { getCachedHasAccess: jest.fn().mockReturnValue(true) };
        }

        throw new Error(`Unexpected registry dependency: ${String(token)}`);
      })
      .compile();
    const service = module.get(AiModelRegistryService);
